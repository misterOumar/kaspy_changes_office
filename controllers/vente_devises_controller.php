...
if (isset($_GET['page']) and !empty($_GET['page']) and $_GET['page'] === 'vente_devises') {
...
if (isset($_POST['bt_enregistrer'])) {
    ...
    $date_vente = strSecur($_POST["date_vente"]);
    ...
    if ($succes) {
        ...
        if ($id_modif == 0) {
            ...
            if (client::Ajouter(
                ...
                $last_client = client::getLast();
                if (vente_devise::Ajouter(
                    $date_vente,
                    $type,
                    $last_client['nom'],
                    $numero_piece,
                    ...
                    $message = "Enregistrement de la vente de dévise éffectué avec succès.";
                    echo json_encode([
                        'success' => 'true',
                        'message' => $message
                    ]);
                ...
                $message = "Erreur lors de l'enregistrement de la vente de dévise.";
                echo json_encode([
                    'success' => 'false',
                    'message' => $message
                ]);
            ...
            if (client::Modifier(
                ...
                $last_client = client::getByID($id_modif);
                if (vente_devise::Ajouter(
                    $date_vente,
                    $type,
                    $last_client['nom'],
...
if (isset($_POST['bt_modifier'])) {
    ...
    // informations de la facturation
    $date_vente = strSecur($_POST["date_vente_modif"]);
    $devise = strSecur($_POST["devise"]);
    ...
    $montant_lettre = convertirNombreEnLettres($total_net);
    $id_vente_modif = strSecur($_POST["id_vente_modif"]);
    ...
    if ($succes) {
        ...
        if (client::Modifier(
            ...
            $client = client::getByID($id_client_modif);
            if (vente_devise::Modifier(
                $date_vente,
                $type,
                $numero_piece,
                $quantite,
                ...
                $message = "Mise à jour de la vente de dévise  éffectué avec succès.";
                echo json_encode([
                    'success' => 'true',
                    'message' => $message
                ]);
            } else {
                $message = "Erreur lors de la mise à jour de la vente de dévise.";
                echo json_encode([
                    'success' => 'false',
                    'message' => $message
                ]);
...
if (isset($_GET['get_client_by_numero_piece'])) {
    ...
    $client = client::getByNumeroPiece($numero_piece);
...
if (isset($_GET['idLast'])) {
    ...
    $transactions = vente_devise::getLast();

    if ($transactions) {
        $total = vente_devise::getCount();
        echo json_encode([
...
if (isset($_GET['idProprietes'])) {
    ...
    $vente_devise = vente_devise::getByID($id);
    $nom_client = $vente_devise["client"];
    $client = client::getByClient($nom_client);
    if ($vente_devise) {
        echo json_encode([
            'vente_devise' => $vente_devise,
            'client' => $client,
        ]);
    } else {
        echo json_encode([
