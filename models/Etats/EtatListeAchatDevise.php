...
class  EtatListeAchatsDevise extends FPDF
{
    public $logoEtat;
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $this->Cell(120, 10, 'JOURNAL DES ACHATS DE DEVISE', 1, 0, 'C');
    ...
    function BasicTable($data)
    {
        ...
        if (count($data) > 0) {
            ...
            foreach ($data as $achat_devise) {
                // Convertir la chaîne de date en objet DateTime
                $date = DateTime::createFromFormat('d/m/Y', $achat_devise['dates']);
                if ($date === false) {
                    $date = new DateTime($achat_devise['dates']);
                }
                ...
                $this->Cell(50, 6,  $achat_devise['emetteur_approvisionnement'], 1, 0, 'L');
                $this->Cell(25, 6,  formaterNombre($achat_devise['quantite']) . " " . $achat_devise['devise_symbole'], 1, 0, 'C');
                $this->Cell(20, 6,  $achat_devise['taux_achat'], 1, 0, 'C');
                $this->Cell(25, 6,  formaterNombre($achat_devise['montant_brut']), 1, 0, 'C');
                $this->Cell(15, 6,  formaterNombre($achat_devise['remise']), 1, 0, 'C');
                $this->Cell(25, 6,  formaterNombre($achat_devise['montant_net']), 1, 0, 'C');
                $this->Ln();
                $i++;
            }
        } else {
            $this->Cell(190, 6,  "Aucune transaction d'achat de devise aujourd'hui.", 1, 0, 'C');
        }
    }
    ...
    public function generatePDF($data, $saveToFile = false)
    {
        ...
        $this->SetTitle('Journal Achat Devise', 1);
        $this->AliasNbPages();
        $this->AddPage('P');
        ...
        if ($saveToFile) {
            $filePath = '../assets/temp/Journal_achats_devise_' . date('d_m_Y') . '.pdf';
