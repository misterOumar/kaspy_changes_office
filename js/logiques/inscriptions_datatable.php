...
<script>
  $(function() {
      ...
      data.map((champ_bd) => {
        ...
        dt_basic.row
          .add({
            ...
            matricule: champ_bd.matricule,
            etudiant: champ_bd.etudiant,
            // status: champ_bd.status,
            // niveuau: champ_bd.niveau,
            // filiere: champ_bd.filiere,
            // type_cours: champ_bd.type_cours,
            classe: champ_bd.classe,
          ...
          },
          {
            ...
            render: function(data, type, full, meta) {
              var $user_img = full['avatar'],
                ...
                $etudiant = full['etudiant'],
                // $status = full['status'],
                // $niveau = full['niveau'],
                // $filiere = full['filiere'],
                // $type_cours = full['type_cours'],
                $classe = full['classe'],
              ...
              var $row_output =
                ...
                '<span class="emp_dates text-truncate fw-bold">' +
                $dates +
                '</span>' +
    ...
    // Flat dates picker
    if (dt_dates_table.length) {
      dt_dates_table.flatpickr({
    ...
    $('#form_ajouter').on('submit', function(e) {
      var $new_dates = $('#dates').val(),
        $new_matricule = $('#matricule').val(),
        $etudiant = $('#etudiant').val(),
        // $status = $('#status').val(),
        // $niveau = $('#niveau').val(),
        // $filiere = $('#filiere').val(),
        // $new_type_cours = ('#type_cours').val(),
        $new_classe = $('#classe').val();
      ...
      if ($new_dates != '') {
        ...
        $.ajax({
          ...
          success: function(result) {
            ...
            if (donnee['success'] === 'existe') {
              ...
              $('#datesHelp').html(donnee['message']);
              $('#datesHelp').removeClass('invisible');

            }
            if (donnee['success'] === 'true') {
              $('#dates').val("");
              $('#datesHelp').html("").addClass('invisible');
              ...
              $.ajax({
                type: "GET",
                data: "idLast=" + true,
                url: "controllers/inscriptions_controller.php",
                success: function(result) {
                  var donnees = JSON.parse(result)
                  if (donnees['last_inscription'] !== 'null') {
                    let lastinscription = donnees['last_inscription']
                    let last_id = lastinscription['id'];
        ...
        if (donnee['success'] === 'false') {
          $('#datesHelp').html(donnee['dates']).removeClass('invisible');
    ...
    $('.datatables-basic tbody').on('click', '.delete-record', function() {
      ...
      Swal.fire({
        ...
        text: "' " + (dt_basic.row($(this).parents('tr')).data().matricule) + " ' sera supprimé définitivement",
        icon: 'warning',
        ...
        if (result.value) {
          Swal.fire({
            ...
            text: "' " + (dt_basic.row($(that).parents('tr')).data().matricule) + " ' a été supprimée avec succès.",
            showConfirmButton: false,
            timer: 1300,
            customClass: {
    ...
    $('.datatables-basic tbody').on('click', '.proprietes', function() {
      ...
      $.ajax({
        ...
        success: function(result) {
          var donnees = JSON.parse(result);
          if (donnees['proprietes_inscriptions'] !== 'null') {

            let proprietes = donnees['proprietes_inscriptions']

            let titre = proprietes['dates'] + ' - ' + proprietes['matricule'];
    ...
    $('#bt_ajouter').on('click', function() {
      //Rédirection vers nouvel inscriptions

      alert('tfthgt')
      location.href = "index.php?page=inscription_new";
    });
  });
</script>
