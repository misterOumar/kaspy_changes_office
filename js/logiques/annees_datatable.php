<script>
  $(function() {
      ...
      data.map((champ_bd) => {
        ...
        dt_basic.row
          .add({
            ...
            id: champ_bd.id,
            annees: champ_bd.annees,
            date_debut: champ_bd.date_debut,
            date_fin: champ_bd.date_fin,
          })
          .draw();
      })
    })
          ...
          },
          {
            ...
            render: function(data, type, full, meta) {
              var $user_img = full['avatar'],
                $annees = full['annees'],
                $date_debut = full['date_debut'],
                $date_fin = full['date_fin'],
    ...
    $('#form_ajouter').on('submit', function(e) {
      var $new_annees = $('#annees').val(),
        $new_date_debut = $('#date_debut').val(),
        $new_date_fin = $('#date_fin').val();
      e.preventDefault()
      ...
      if ($new_annees != '') {
        ...
        $.ajax({
          ...
          success: function(result) {
            ...
            if (donnee['success'] === 'existe') {
              ...
              $('#anneesHelp').html(donnee['message']);
              $('#anneesHelp').removeClass('invisible');

            }
            if (donnee['success'] === 'true') {
              $('#annees').val("");
              $('#anneesHelp').html("").addClass('invisible');
              ...
              $.ajax({
                type: "GET",
                data: "idLast=" + true,
                url: "controllers/annees_controller.php",
                success: function(result) {
                  var donnees = JSON.parse(result)
                  if (donnees['last_annee'] !== 'null') {
                    let lastannee = donnees['last_annee']
                    let last_id = lastannee['id'];
        ...
        if (donnee['success'] === 'false') {
          $('#anneesHelp').html(donnee['annees']).removeClass('invisible');
    ...
    $('.datatables-basic tbody').on('click', '.delete-record', function() {
      ...
      Swal.fire({
        ...
        text: "' " + (dt_basic.row($(this).parents('tr')).data().annees) + " ' sera supprimé définitivement",
        icon: 'warning',
        ...
        if (result.value) {
          Swal.fire({
            ...
            text: "' " + (dt_basic.row($(that).parents('tr')).data().annees) + " ' a été supprimée avec succès.",
        ...
        $('.datatables-basic tbody').on('click', '.item-edit', function() {
            ...
            $.ajax({
                ...
                success: function(result) {
                    var donnees = JSON.parse(result);
                    // alert(result)
                    if (donnees['proprietproprietes_anneeses_batiment'] != 'null') {
                        // Remplir le formulaire
                        let annees = donnees['proprietes_annees'];
                        $('#idModif').val(annees['id']);
                        $('#annees_modif').val(annees['annees']);
                        $('#date_debut_modif').val(annees['date_debut']);
                        $('#date_fin_modif').val(annees['date_fin']);
    ...
    $('.datatables-basic tbody').on('click', '.proprietes', function() {
      ...
      $.ajax({
        ...
        success: function(result) {
          var donnees = JSON.parse(result);
          if (donnees['proprietes_annees'] !== 'null') {

            let proprietes = donnees['proprietes_annees']

            let titre = proprietes['annees'];
