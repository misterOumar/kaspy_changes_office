 ...
 <script>
     ...
     boutonLirePressePapier.addEventListener('click', async () => {
         try {
             ...
             if (valeur.includes("Vous avez recu")) {
                 // Extraction et analyse du texte
                 const RESULTAT_extraireInformations_RETRAIT = extraireInformations_RETRAIT(valeur);

                 $('#radio_retrait').prop('checked', true);
                 $('#solde_t').val(RESULTAT_extraireInformations_RETRAIT.solde);
                 $('#tel_cli').val(RESULTAT_extraireInformations_RETRAIT.numero);
                 $('#montant').val(RESULTAT_extraireInformations_RETRAIT.montant);
                 $('#id_transaction').val(RESULTAT_extraireInformations_RETRAIT.idTransaction);

             } else if (valeur.includes("Vous avez envoye")) {
                 // Extraction et analyse du texte
                 const RESULTAT_extraireInformations_DEPOT = extraireInformations_DEPOT(valeur);

                 // Collage dans les cellules
                 $('#radio_depot').prop('checked', true);
                 $('#solde_t').val(RESULTAT_extraireInformations_DEPOT.solde);
                 $('#montant').val(RESULTAT_extraireInformations_DEPOT.montant);
                 $('#tel_cli').val(RESULTAT_extraireInformations_DEPOT.numero);
                 $('#id_transaction').val(RESULTAT_extraireInformations_DEPOT.idTransaction);
             }
         } catch (error) {
     ...
     // Extration de Texte RETRAIT MOOV MONEY
     function extraireInformations_RETRAIT(texte) {
         // Utiliser une expression régulière pour extraire les informations nécessaires
         const regex = /recu (\d+\s?\d*) FCFA de (\d+)\.Commission de la transaction (\d+) FCFA\. Votre nouveau solde Moov money est de (\d+\s?\d*) FCFA\. Ref :(\w+\.\w+\.\w+)/;
     ...
     // Extration de Texte DEPOT MOOV MONEY
     function extraireInformations_DEPOT(texte) {
         let contenuVariable = texte

         // Trouver le montant
         let montantIndexDebut = contenuVariable.indexOf("envoye") + 6;
         ...
         // Trouver le numéro
         let numeroIndexDebut = contenuVariable.indexOf("numero") + 6;
         let numeroIndexFin = contenuVariable.indexOf(".Commission", numeroIndexDebut);
         let numero = contenuVariable.substring(numeroIndexDebut, numeroIndexFin).trim();
         numero = numero.replace(/\s/g, '');

         // Trouver le solde
         let soldeIndexDebut = contenuVariable.indexOf("Votre nouveau solde Moov money est de") + 38;
