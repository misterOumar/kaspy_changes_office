    ...
    $(window).on('load', function() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                var montant_total_payer_mois_en_cours = donnees['montant_total_payer_mois_en_cours']
                var montant_total_reste_mois_en_cours = donnees['montant_total_reste_mois_en_cours']
                // alert(montant_total_payer_mois_en_cours)

                //REMPLISSAGE DANS LE TABLEAU 
                var donnee_tableau_payer = [];
                var donnee_tableau_reste = [];
                var tableau_mois = [];
                for (var i = 0; i < donnees_montant_payer.length; i++) {
                    donnee_tableau_payer.push(donnees_montant_payer[i]['somme_montant_payer']);
                    donnee_tableau_reste.push(-parseInt(donnees_reste_a_payer[i]['somme_reste_a_payer']));
                    tableau_mois.push(donnees_montant_payer[i]['mois_fr'])
                }
                // alert(donnee_tableau_reste)
                ...
                revenueReportChartOptions = {
                    ...
                    series: [{
                            name: 'Payer',
                            data: donnee_tableau_payer, //TABLEAU DES RECOUVREMENT PAYER 
                        },
                        {
                            name: 'Reste a payer',
                            data: donnee_tableau_reste //TABLEAU DES RECOUVREMENT RESTANT A PAYER 
                        }
                    ],
                    dataLabels: {
                        enabled: false
                    ...
                    xaxis: {
                        categories: tableau_mois, //TABLEAU DES MOIS
                        labels: {
                            style: {
                ...
                earningsChartOptions = {
                    ...
                    series: [parseInt(montant_total_reste_mois_en_cours), parseInt(montant_total_payer_mois_en_cours)],
                    legend: {
                        show: false
                    },
                                ...
                                labels: {
                                    ...
                                    total: {
                                        ...
                                        formatter: function(w) {
                                            return (parseInt(montant_total_reste_mois_en_cours) + parseInt(montant_total_payer_mois_en_cours));
                                        }
                                    }
                                }
                            }
                ...
                budgetChartOptions = {
                    ...
                    series: [{
                            data: donnee_tableau_payer
                        },
                        {
                            data: donnee_tableau_reste
                        }
                    ],
                    tooltip: {
                        enabled: false
        ...
        //GRAPHIQUE EN DONUT DES RECOUVREMENT EN FONCTION DU MOIS SELECTIONNEES
        $('#MoisGraph').on('change', function() {
            // alert($('#MoisGraph').val());
            // Détruire l'instance du graphique existant
            if (typeof earningsChart !== 'undefined') {
                earningsChart.destroy();
            }
            $.ajax({
                type: "GET",
                data: "MoisGraph=" + $('#MoisGraph').val(), //Envois de l'id selectionné
                url: "controllers/home_controller.php",
                success: function(result) {
                    var donnees = JSON.parse(result);
                    if (donnees['success'] === 'true') {

                        var montant_total_payer_mois_choisi = donnees['montant_total_payer_mois_choisi']
                        var montant_total_reste_mois_choisi = donnees['montant_total_reste_mois_choisi']
                        // console.log(donnees['montant_total_payer_mois_choisi']);
                        ...
                        earningsChartOptions = {
                            ...
                            series: [parseInt(montant_total_reste_mois_choisi), parseInt(montant_total_payer_mois_choisi)],
                            legend: {
                                show: false
                            },
                            comparedResult: [2, -3, 8],
                                    ...
                                    donut: {
                                        labels: {
                                            ...
                                            total: {
                                                ...
                                                label: 'Totales',
                                                formatter: function(w) {
                                                    return (parseInt(montant_total_reste_mois_choisi) + parseInt(montant_total_payer_mois_choisi));
                                                }
                                            }
                                        }
                                    }
                        ...
                        $("#PayerMoisChoisi").text(montant_total_payer_mois_choisi);
                        $("#ImpayerMoisChoisi").text(montant_total_reste_mois_choisi);
                        earningsChart = new ApexCharts($earningsChart, earningsChartOptions);
                        earningsChart.render();
                    }
