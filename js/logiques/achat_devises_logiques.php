...
<script>
    ...
    function handleDeviseChange(elementId, tauxAchatId) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (donnee['success'] === 'true') {
                        ...
                        $('#' + tauxAchatId).val(devise['taux_achat']);
                    }
                }
            });
        });
    ...
    function handleQuantiteTauxRemiseChange(quantiteId, tauxId, remiseId, totalBrutId, totalNetId) {
        $('#' + quantiteId + ', #' + tauxId + ', #' + remiseId).on('input', function() {
            ...
            var taux_achat = parseFloat($('#' + tauxId).val());
            var remise = parseFloat($('#' + remiseId).val());
            let total_brut;
            let total_net;

            total_brut = quantite * taux_achat;
            total_net = total_brut - remise

            if (!isNaN(quantite) && !isNaN(taux_achat)) {
    ...
    $(" #date_achat , #date_achat_modif").flatpickr({
        defaultDate: 'today',
        dateFormat: "Y-m-d",
    })

    // charger taux achat lorsqu'on choisi la devise
    handleDeviseChange('devise', 'taux_achat');

    // actualiser les montant en cas de changement
    handleQuantiteTauxRemiseChange("quantite", "taux_achat", "remise", "total_brut", "total_net")
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (donnee['success'] === 'true') {
                    ...
                    $('#taux_achat_conversion').val(devise['taux_achat'])
    ...
    $('#bt_convertir').on('click', function() {
        var taux_achat_conversion = parseFloat($('#taux_achat_conversion').val());
        ...
        quantite = montant_conversion / taux_achat_conversion;
        total_net = total_brut - remise

        if (!isNaN(taux_achat_conversion) && !isNaN(montant_conversion)) {

            $('#taux_achat').val(taux_achat_conversion); // afficher le montant avec deux décimales
    ...
    $('#telephone, #telephone_modif').on('keyup', function(e) {
    ...
    $('#taux, #taux_modif').on('keyup', function(e) {
        var selectedId = $(this).attr('id');
        var taux = $(this).val();
    ...
    $('#taux_achat, #taux_achat_modif').on('keyup', function() {
        var selectedId = $(this).attr('id');
        var taux = $(this).val();
        if ($.isNumeric(taux) === false) {
            formValide = false;
    ...
    $('#bt_enregistrer').on('click', function(e) {
        ...
        if (formValide) {
            ...
            //             //location.href = "index.php?page=inscriptions";
            //         }
            //     }
            // });
        }
    ...
    // charger taux achat lorsqu'on choisi la devise
    handleDeviseChange('devise_modif', 'taux_achat_modif');

    // actualiser les montant en cas de changement
    handleQuantiteTauxRemiseChange("quantite_modif", "taux_achat_modif", "remise_modif", "total_brut_modif", "total_net_modif")
