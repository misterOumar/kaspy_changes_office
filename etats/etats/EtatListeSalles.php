...
class EtatListeSalles extends FPDF
{
    public $logoEtat;
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $this->Cell(95, 10, 'LISTE DES SALLES', 1, 0, 'C');
    ...
    function BasicTable($data)
    {
        ...
        $this->Cell(57, 7, "Type", 1, 0, 'C');
        $this->Cell(20, 7, "Capacité", 1, 0, 'C');
        $this->Ln();

        $i = 1;
        $this->SetFont('Helvetica', '', 9);
        foreach ($data as $salles) {
            $this->Cell(13, 6, $i, 1, 0, 'C');
            $this->Cell(100, 6, $salles['nom'], 1);
            $this->Cell(57, 6, $salles['type_salle'], 1, 0, 'C');
            $this->Cell(20, 6, $salles['capacite'], 1, 0, 'C');
            $this->Ln();
            $i++;
        }
    }
...
for ($num = 0; $num < count($salles); $num++) {
    array_push(
        $data,
        $salles[$num]
    );
...
$pdf->SetTitle('Liste des salles', 1);
