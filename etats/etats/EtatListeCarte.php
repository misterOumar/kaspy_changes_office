...
class   EtatListeCarte extends FPDF
{
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $this->Cell(200, 10, 'LISTE DES CARTES', 1, 0, 'C');
    ...
    function BasicTable($data)
    {
        ...
        $this->Cell(50, 7, "CUSTOMER ID", 1, 0, 'C');
        $this->Cell(40, 7, "D'ACTIVATION ", 1, 0, 'C');
        $this->Cell(60, 7, "DATE D'EXPIRATION", 1, 0, 'C');
        $this->Cell(40, 7, "TYPE DE CARTE", 1, 0, 'C');
        $this->Cell(40, 7, "DUREE(jours)", 1, 0, 'C');  
        ...
        foreach ($data as $batiment) {
            ...
            $this->Cell(50, 6,  $batiment['customer_id'], 1, 0, '');
            $this->Cell(40, 6,  $batiment['date_activation'], 1, 0, 'C');
            $this->Cell(60, 6,  $batiment['date_expiration'], 1, 0, 'C');
            $this->Cell(40, 6,  $batiment['type_carte'], 1, 0, 'C');
            $this->Cell(40, 6,  $batiment['duree'], 1, 0, 'C');  
            $this->Cell(30, 6,  $batiment['status'] == 0 ? "EN STOCK": "VENDUE", 1, 0, 'C');
            $this->Ln();
            $i++;
        }
    }
