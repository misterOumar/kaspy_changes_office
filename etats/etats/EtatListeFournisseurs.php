...
class EtatListeFournisseurs extends FPDF
{
    public $logoEtat;
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $this->Cell(95, 10, 'LISTE DES FOURNISSEURS', 1, 0, 'C');
    ...
    function BasicTable($data)
    {
        ...
        $this->Cell(100, 7, "Raison sociale", 1, 0, 'C');
        $this->Cell(40, 7, "Téléphone", 1, 0, 'C');
        $this->Cell(35, 7, "Interlocuteur", 1, 0, 'C');
        $this->Ln();
        ...
        foreach ($data as $fournisseurs) {
            ...
            $this->Cell(100, 6, $fournisseurs['raison_sociale'], 1);
            $this->Cell(40, 6, $fournisseurs['tel'], 1, 0, 'C');
            $this->Cell(35, 6, $fournisseurs['interlocuteur'], 1, 0, 'C');
            $this->Ln();
            $i++;
        }
    }
...
for ($num = 0; $num < count($fournisseurs); $num++) {
    array_push(
        $data,
        $fournisseurs[$num]
    );
...
$pdf->SetTitle('Liste des fournisseurs', 1);
