...
class EtatListeLocataires extends FPDF
{
    public $logoEtat;
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $this->Cell(200, 10, 'LISTE DES LOCATAIRES', 1, 0, 'C');
    ...
    function BasicTable($data)
    {
        ...
        $this->Cell(50, 7, "Nom prenom", 1, 0, 'C');
        $this->Cell(50, 7, "Type", 1, 0, 'C');
        $this->Cell(45, 7, "Type piece", 1, 0, 'C');
        $this->Cell(35, 7, "Numero piece", 1, 0, 'C');
        $this->Cell(20, 7, "Telephone", 1, 0, 'C');
        $this->Cell(70, 7, "Email", 1, 0, 'C');;
        $this->Ln();
        ...
        foreach ($data as $batiment) {
            ...
            $this->Cell(50, 6, $batiment['nom_prenom'], 1, 0, '');
            $this->Cell(50, 6,  $batiment['type_locataire'], 1, 0, 'C');
            $this->Cell(45, 6,  $batiment['type_piece'], 1, 0, 'C');
            $this->Cell(35, 6,  $batiment['numero_piece'], 1, 0, 'C');
            $this->Cell(20, 6,  $batiment['telephone'], 1, 0, 'C');
            $this->Cell(70, 6,  $batiment['email'], 1, 0, 'C');
            $this->Ln();
            $i++;
        }
    }
...
$data = array();
$locataires = locataires::getAll();
for ($num = 0; $num < count($locataires); $num++) {
    array_push(
        $data,
        $locataires[$num]
    );
...
$pdf->SetTitle('Liste des Locataires', 1);
...
function generatePDF($data)
{
    ...
    $pdf->Cell(0, 10, 'Liste des LOcataires', 0, 1, 'C');
    ...
    // Génération du PDF
    $pdf->Output('liste_locataires.pdf', 'D'); // 'D' pour afficher le téléchargement du fichier
}
