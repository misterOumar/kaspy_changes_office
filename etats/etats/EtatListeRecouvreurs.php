...
require_once("../models/Recouvreurs.php");

class EtatListeRecouvreurs extends FPDF
{
    public $logoEtat;
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $this->Cell(200, 10, 'LISTE DES RECOUVREURS', 1, 0, 'C');
    ...
    function BasicTable($data)
    {
        ...
        $this->Cell(140, 7, "Nom prenom", 1, 0, 'C');
        $this->Cell(50, 7, "Type piece", 1, 0, 'C');
        $this->Cell(60, 7, "Numero piece", 1, 0, 'C');
        ...
        foreach ($data as $batiment) {
            ...
            $this->Cell(140, 6, $batiment['nom_prenom'], 1, 0, '');
            $this->Cell(50, 6,  $batiment['type_piece'], 1, 0, 'C');
            $this->Cell(60, 6,  $batiment['numero_piece'], 1, 0, 'C');
            $this->Ln();
            $i++;
        }
    }
...
$data = array();
$recouvreurs = recouvreurs::getAll();
for ($num = 0; $num < count($recouvreurs); $num++) {
    array_push(
        $data,
        $recouvreurs[$num]
    );
...
$pdf->SetTitle('Liste des Recouvreurs', 1);
...
function generatePDF($data)
{
    ...
    $pdf->Cell(0, 10, 'Liste des Recouvreurs', 0, 1, 'C');
    ...
    // Génération du PDF
    $pdf->Output('liste_recouvreurs.pdf', 'D'); // 'D' pour afficher le téléchargement du fichier
}
