...
class EtatListeAppartements extends FPDF
{
    public $logoEtat;
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $this->Cell(200, 10, 'LISTE DES APPARTEMENTS', 1, 0, 'C');
    ...
    function BasicTable($data)
    {
        ...
        $this->Cell(60, 7, "Batiment", 1, 0, 'C');
        $this->Cell(25, 7, "Type", 1, 0, 'C');
        $this->Cell(35, 7, "Locataire", 1, 0, 'C');
        $this->Cell(50, 7, "Recouvreur", 1, 0, 'C');
        $this->Cell(25, 7, "Gerance", 1, 0, 'C');
        $this->Ln();
        ...
        foreach ($data as $batiment) {
            ...
            $this->Cell(65, 6, $batiment['libelle'], 1, 0, '');
            $this->Cell(60, 6,  $batiment['batiment'], 1, 0, 'C');
            $this->Cell(25, 6,  $batiment['type_appartement'], 1, 0, 'C');
            $this->Cell(35, 6,  $batiment['locataire'], 1, 0, 'C');
            $this->Cell(50, 6,  $batiment['recouvreur'], 1, 0, 'C');
            $this->Cell(25, 6,  $batiment['commission_gerance'], 1, 0, 'C');
            $this->Ln();
            $i++;
        }
    }
...
function generatePDF($data)
{
    ...
    // Génération du PDF
    $pdf->Output('liste_appartements.pdf', 'D'); // 'D' pour afficher le téléchargement du fichier
}
