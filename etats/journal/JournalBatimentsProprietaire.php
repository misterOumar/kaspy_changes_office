...
class JournalBatimentsProprietaire extends FPDF
{
    public $logoEtat;
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $this->Cell(150, 10, 'JOURNAL DES BATIMENTS PAR PROPRIETAIRE', 1, 0, 'C');
    ...
    function BasicTable($data)
    {
        ...
        $this->Cell(60, 7, "Libelle", 1, 0, 'C');
        $this->Cell(45, 7, "Type", 1, 0, 'C');
        $this->Cell(70, 7, "Propriétaire", 1, 0, 'C');
        $this->Ln();
        ...
        foreach ($data as $batiment) {
            ...
            $this->Cell(60, 6, $batiment['libelle'], 1, 0, '');
            $this->Cell(45, 6,  $batiment['type_batiment'], 1, 0, 'C');
            $this->Cell(70, 6,  $batiment['nom_prenom'] , 1, 0, 'C');
            $this->Ln();
            $i++;
        }
    }
...
$pdf = new JournalBatimentsProprietaire();
...
function generatePDF($data)
{
    ...
    $pdf->Cell(0, 10, 'Journal des batiments par propriétaire', 0, 1, 'C');
    ...
    // Génération du PDF
    $pdf->Output('journal_batiment_proprietaire.pdf', 'D'); // 'D' pour afficher le téléchargement du fichier
}
