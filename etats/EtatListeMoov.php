...
if (isset($_POST['bt_moov'])) {
    ...
    $transactions_moov = moov::getAllBetween2Date($date_debut, $date_fin);
    //  var_dump($transactions_moov);
    ...
    if (count($transactions_moov) > 0) {
        foreach ($transactions_moov as $transaction) {
            ...
            $pdf->Cell(30, 6, formaterNombre($transaction['solde_total']), 1); 
            $pdf->Ln();
            $i++;
    ...
    end($transactions_moov);
    // Récupérer la valeur du dernier élément
    $lastTransaction = current($transactions_moov);
    ...
    header('Content-Disposition: inline; filename="JournalDesTransactionsMoovMoney.pdf"');
