...
class EtatListeOperation extends FPDF
{
    ...
    function Header()
    {
        if ($this->PageNo() == 1) {
            ...
            $titre = '   RAPPORT DE SUIVI DES OPERATIONS MONEY GRAM   ';
    ...
    function BasicTable($data)
    {
        ...
        foreach ($data as $transaction) {
            ...
            $date = DateTime::createFromFormat('d-m-Y', $transaction['dates']);
            ...
            $this->Cell(30, 6,$transaction['dates'], 1, 0, 'C');

            $this->SetFont('Helvetica', 'B', 10);
            $this->Cell(27, 6,$magasin, 1,0,'C');
            ...
            $this->Cell(28, 6,number_format($transaction['montant_paye'], 2, '.', ''), 1, 0, 'C');
            $this->Cell(20, 6,number_format($transaction['montant_paye'], 2, '.', ''), 1, 0, 'C');
...
$data = array();
 $magasin = $_SESSION["KaspyISS_bureau"];
$transactions_moneyGram = moneygram::getAllRapport();
// var_dump($transactions_moneyGram );
for ($num = 0; $num < count($transactions_moneyGram); $num++) {
    array_push(
        $data,
        $transactions_moneyGram[$num]
    );
}
