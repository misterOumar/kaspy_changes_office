 <?php
    ...
    if (isset($_GET['idRecuImprimer'])) {
        ...
        require_once("../models/Emetteur_appro.php");

        $id_achat_devise = intval($_GET['idRecuImprimer']);
        $achat_devise = achat_devise::getByID($id_achat_devise);

        $client = emetteur_appro::getByClient($achat_devise['emetteur_approvisionnement']);
        ...
        $date = DateTime::createFromFormat('d/m/Y H:i', $achat_devise['date_creation']);

        if ($date === false) {
            // Si la conversion échoue, essayer un autre format
            $date = new DateTime($achat_devise['date_creation']);
        }
        ...
        function generateReceipt($pdf, $logo, $entreprise, $annee, $dateFormatee, $client, $achat_devise)
        {
            // Logo
            ...
            $pdf->Cell(100, 14, "REÇU D'ACHAT DE DEVISE", 1, 0, 'C');
            ...
            $pdf->Ln();
            $pdf->Cell(102);
            $bordereau_numero = substr(date("Y"), -2) . "A000".$achat_devise["id"];
            $pdf->SetFont('Arial', 'B', 10);
            ...
            $pdf->SetFont('Arial', 'I', 10);
            $pdf->Cell(35, 8, number_format($achat_devise['quantite'], 0, '', ' '), 'LTB', 0, 'L');
            $pdf->Cell(20, 8, $achat_devise['devise'], 'RTB', 0, 'R');
            ...
            $pdf->SetFont('Arial', 'I', 10);
            $pdf->Cell(15, 8, $achat_devise['taux_achat'], 1, 0, 'C');
            ...
            $pdf->SetFont('Arial', 'I', 10);
            $pdf->Cell(40, 8, number_format($achat_devise['montant_brut'], 0, '', ' '), 1, 0, 'C');
            ...
            $pdf->Cell(2);
            $pdf->Cell(91, 8, number_format($achat_devise['montant_net'], 0, '', ' '), 1, 0, 'C');
            ...
            $pdf->Ln(6);
            $pdf->SetFont('Arial', 'B', 10);
            $pdf->Cell(187, 5, $achat_devise['montant_lettre'], 0, 0, 'L');
        }


        // recu du client
        generateReceipt($pdf, $logo, $entreprise, $annee, $dateFormatee, $client, $achat_devise);
        ...
        $pdf->SetTitle('RECU ACHAT DE DEVISES', 1);
        $pdf->Output('I', 'ACHAT_DEVISE_'.$achat_devise['date_creation'].'.pdf');
